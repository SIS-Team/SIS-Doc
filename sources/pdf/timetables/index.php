<?php
	 ...
	 * Beschreibung:
	 *	Erzeugt die PDF-Ausgabe fuer den Stundenplan
	 */	
...
if($permission == "root" or $permission == "admin") {  //Schueler sehen nur ihren eigenen Stundenplan
	$mode ="admin";
...
if($mode == 'student') { //verhindert das Schueler Lehrerstundenplaene sehen
...
{//um individuellen Inhalt in Kopf- bzw. Fusszeile zu ermoeglichen
	//Kopfzeile
	function Header()
	{
	    //Logo
		...
		$this->Cell('130','25','HTL Anichstrasse');
	    //Zeilenumbruch
	    $this->Ln(20);
	}
...
if(isset($results)){
	for($j=0;$j<count($results);$j++){ //alle Lessons durchlaufen
		...
		while($startHour <= $results[$j]['endHour']) //fuer Stunden die laenger als eine Stunde dauern
		{	
			...
			else $hours[$startHour][$results[$j]['weekday']] = $results[$j]['suShort']; // sonst Eintrag ueberschreiben
		$startHour++;
		}
	}
}
...
for($i=$start;$i<$end;$i++){ //Stundenplanausgabe
	...
	for($j=1;$j<6;$j++){
		...
		if(isset($hours[$i][$day[$j]])){
			...
			//Multicell, da Eintrag laenger als eingestellte Breite der Spalte sein kann 
			//RLT = Rand oben, links und rechts
			if($pdf->GetY() > $newY)$newY=$pdf->GetY();	//neue Y-Position speichern
			//Y-Position aendert sich, da nach Multicell automatisch in einer neuen Zeile begonnen wird
		}
		else {
	...
	$pdf->SetY($y);	//da bei set Y zum Zeilenbeginn zurueckgesprungen wird ,muss dies nocheinmal ausgefuehrt werden

 	$newHeight = $newY-$y; //Hoehe der zuerstellenden Zelle
	$pdf->Cell('25',$newHeight,'','RLB'); //Zelle mit neure Hoehe und Rand unten, rechts und links
	for($j=1;$j<6;$j++)
	{
